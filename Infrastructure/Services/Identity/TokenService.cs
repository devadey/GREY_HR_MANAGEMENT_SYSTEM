...
namespace Infrastructure.Services.Identity
{
    public class TokenService : ITokenService
    {
        ...
        private string GenerateEncrytedToken(SigningCredentials signingCredentials, IEnumerable<Claim> claims)
        {
            ...
            var encryptedToken = tokenHandler.CreateToken(token);
            var tokenEncrypted = tokenHandler.WriteToken(encryptedToken);
            return tokenEncrypted;
        }

        private SigningCredentials GetSigningCredentials()
        {
            var secret = new SymmetricSecurityKey(Encoding.UTF8.GetBytes(_appConfiguration.Secret));
